...
describe Coinbase::OAuthClient do
  BASE_URI = 'http://fake.com/api/v1' # switching to http (instead of https) seems to help mock_requestWeb

  before :all do
    client_options = {base_uri: BASE_URI, refresh_token: 'refresh token', expires_at: 'some timestamp'}
    @c = Coinbase::OAuthClient.new 'api key', 'api secret', 'access token', client_options
  end

  # Auth and Errors

  it "raise errors" do
    mock_request :get, '/account/balance', {error: "some error"}
    expect{ @c.balance }.to raise_error(Coinbase::Client::Error, 'some error')
    mock_request :get, '/account/balance', {errors: ["some", "error"]}
    expect{ @c.balance }.to raise_error(Coinbase::Client::Error, 'some, error')
  end
  ...
  it "should get balance" do
    mock_request :get, '/account/balance', {amount: "50.00000000", currency: 'BTC'}
    @c.balance.should == 50.to_money('BTC')
  end

  it "should get a receive address" do
    mock_request :get, '/account/receive_address', {address: "muVu2JZo8PbewBHRp6bpqFvVD87qvqEHWA", callback_url: nil}
    a = @c.receive_address
  ...
  it "should generate new receive addresses" do
    mock_request :post, '/account/generate_receive_address', {address: "mmxJyTdxHUJUDoptwLHAGxLEd1rAxDJ7EV", callback_url: "http://example.com/callback"}
  ...
  it "should create a new button" do
    ...
    mock_request :post, '/buttons', response
  ...
  it "should create order for the button" do
    ...
    mock_request :post, '/buttons/93865b9cae83706ae59220c013bc0afd/create_order', response
  ...
  it "should get transaction list" do
    ...
    mock_request :get, '/transactions', response
    r = @c.transactions
  ...
  it "should send money" do
    ...
    mock_request :post, '/transactions/send_money', response
  ...
  it "should request money" do
    ...
    mock_request :post, '/transactions/request_money', response
  ...
  it "should resend requests" do
    response = {"success"=>true}
    mock_request :put, "/transactions/501a3554f8182b2754000003/resend_request", response
  ...
  it "should cancel requests" do
    response = {"success"=>true}
    mock_request :delete, "/transactions/501a3554f8182b2754000003/cancel_request", response
  ...
  it "should resend requests" do
    response = {"success"=>true}
    mock_request :put, "/transactions/501a3554f8182b2754000003/complete_request", response
  ...
  it "should let you create users" do
    ...
    mock_request :post, "/users", response
  ...
  it "should let you get buy and sell prices" do
    ...
    mock_request :get, "/prices/buy", response
    r = @c.buy_price 1
    r.to_f.should == 13.84

    mock_request :get, "/prices/sell", response
    r = @c.sell_price 1
    r.to_f.should == 13.84
  end
  ...
  it "should let you buy bitcoin" do
    ...
    mock_request :post, "/buys", response
    r = @c.buy! 1
    r.success?.should == true
  ...
  it "should let you sell bitcoin" do
    ...
    mock_request :post, "/sells", response
    r = @c.sell! 1
    r.success?.should == true
  ...
  it "should get recent transfers" do
    ...
    mock_request :get, "/transfers", response
    r = @c.transfers
  ...
  def mock_request method, path, body
    ...
    expect(mock_response).to receive(:body).and_return(body.to_json)
    mock_token = Object.new
    expect(mock_token).to receive(:request).with(method, path, anything).and_return(mock_response)
    expect(@c).to receive(:get_oauth_token).and_return(mock_token)
  end
end
